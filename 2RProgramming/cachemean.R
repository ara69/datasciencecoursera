...
hospital <- read.hospital.data("heart attack")
state <- "CA"
state.data <- na.omit(hospital[hospital$state == state, ])
rates <- state.data$rate

data <- makeVector(rates)
cachemean(data)
# Calling for the same data
cachemean(data)

# Call for different state
state <- "TX"
state.data <- na.omit(hospital[hospital$state == state, ])
rates <- state.data$rate
cachemean(makeVector(rates))
# Calling for the same state
cachemean(makeVector(rates))
